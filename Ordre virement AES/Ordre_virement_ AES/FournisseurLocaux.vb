                                            ...
                                            If Trim(OledtFournisseur.Rows(j).Item("OidBordereauReglement").ToString) <> "" And Trim(OledtFournisseur.Rows(j).Item("OidRoleTiers").ToString) <> "" Then
                                                ...
                                                If Convert.IsDBNull(ReferenceFactureInterne) = False Then
                                                    ...
                                                    For m As Integer = 1 To CInt(ReferenceFactureInterne.NombOrd)
                                                        If vDeviseMonetaire.CodeISO = "CFA" Or vDeviseMonetaire.CodeISO = "XAF" Then
                                                            Edition_Ordre_Virement(ReferenceInterne, OledtFournisseur.Rows(j).Item("OidcompteBancaireEts").ToString, Txt_Aes.Text, m, Format(DTdate.Value, "dd MMM yyyy"), Trim(OledtFournisseur.Rows(j).Item("Caption").ToString), OledtFournisseur.Rows(j).Item("OidcompteBancaireTiers").ToString, Format((SomMnt - (SomIs + SomTva)), "##,##0"))
                                                        Else
                                                            Edition_Ordre_Virement_Devise(ReferenceInterne, OledtFournisseur.Rows(j).Item("OidcompteBancaireEts").ToString, Txt_Aes.Text, m, Format(DTdate.Value, "dd MMM yyyy"), Trim(OledtFournisseur.Rows(j).Item("Caption").ToString), OledtFournisseur.Rows(j).Item("OidcompteBancaireTiers").ToString, (SomMnt - (SomIs + SomTva)), vDevise)
                                                        End If
                                                    Next m
                                                    If EstEditer = True Then
                                                        ClasMan.BeginTran(True)
